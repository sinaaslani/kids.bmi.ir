...
namespace MarkupParser
{
    ...
    public class MarkupDocument
    {
        ...
        private static List<string> knownInlineTags = null;
        /// <summary>
        ...
        private bool fixBadlyFormedInlineTags, useCaching, caseSensitiveComparisons;
        private StringComparison stringComparison = StringComparison.OrdinalIgnoreCase;
        private string text;
        private List<MarkupTag> rootTags = new List<MarkupTag>();
        ...
        public MarkupDocument(string markupText, bool fixBadlyFormedInlineTags, bool useCaching, bool caseSensitiveComparisons)
        {
            this.CaseSensitiveComparisons = caseSensitiveComparisons;
            this.fixBadlyFormedInlineTags = fixBadlyFormedInlineTags;
            this.useCaching = useCaching;
            this.Text = markupText;
        }
        #endregion
        ...
        private void ParseMarkup()
        {
            ...
            List<MarkupTag> tags = new List<MarkupTag>();
            ...
            List<int> usedClosingTags = new List<int>();
            if (this.fixBadlyFormedInlineTags)
            {
            ...
            Queue<MarkupTag> tagQueue = new Queue<MarkupTag>(tags);
            Stack<MarkupTag> tagStack = new Stack<MarkupTag>(1);
            List<MarkupTag> processedTags = new List<MarkupTag>(tagCount + 1);
            MarkupTag currentTag = null, parentTag = null;
            List<int> closedTagIndices = new List<int>(tagCount + 1);
            int nestingLevel = 0;
            while (tagQueue.Count > 0)
            {
                ...
                else if (currentTag.IsClosingTag) // normal close tag
                {
                    ...
                    for (int i = processedTags.Count - 1; i > 0; i--)
                    {
                        ...
                        if (currentTag.Tag.Equals("/" + processedTags[i].Tag, stringComparison) && !closedTagIndices.Contains(i))
                        {
            ...
            for (int i = 0; i < tagCount; i++)
            {
                ...
                for (int j = i + 1; j <= tagCount; j++)
                {
                    ...
                    while ((!tj.IsClosingTag || tj.NestingLevel != ti.NestingLevel || usedClosingTags.Contains(j)) && ++j <= tagCount);
                    // skip these kinds of tags; we're looking for a normal closing tag for the current normal opening tag
            ...
            List<MarkupTag> tagsToRemove = new List<MarkupTag>();
            ...
            #endregion
#if DEBUG
            totalElapsed = sw1.Elapsed + sw2.Elapsed + sw3.Elapsed + sw4.Elapsed + sw5.Elapsed + sw6.Elapsed + sw7.Elapsed;
#endif
        }
        ...
        private MarkupTag[] GetTagsByAttribute(string attributeName, string attributeValue)
        {
            ...
            Stack<MarkupTag> processStack = new Stack<MarkupTag>();
            MarkupTag rootTag = null, t = null, currentTag = null;
            int i;
            for (i = 0; i < this.rootTags.Count; i++)
            {
                rootTag = rootTags[i];
            ...
            while (processStack.Count > 0)
            {
                ...
                for (i = 0; i < currentTag.Children.Count; i++)
                {
                    ...
                    if (t.Comment || t.IsClosingTag || (t.Children.Count == 0 && t.Attributes.Count == 0)) // we'll skip these types as they won't have attributes to search
                        continue;
                    processStack.Push(t);
                }
            }
        ...
        private static MarkupDocument LoadCore(string markupText, bool fixBadlyFormedInlineTags)
        {
            ...
            return new MarkupDocument(markupText, fixBadlyFormedInlineTags, (markupText.Length > CACHE_BOUNDARY ? true : false), true);
        }
        #endregion
