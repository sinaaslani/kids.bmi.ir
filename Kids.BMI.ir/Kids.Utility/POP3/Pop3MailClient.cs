...
namespace Kids.Utility.POP3
{
    ...
    public class Pop3MailClient
    {
        ...
        protected void CallWarning(string methodName, string response, string warningText,
                                   params object[] warningParameters)
        {
            try
            {
        ...
        protected void EnsureState(Pop3ConnectionStateEnum requiredState)
        {
            if (pop3ConnectionState != requiredState)
            {
                ...
                throw new Pop3Exception("GetMailboxStats only accepted during connection state: " + requiredState +
                                        "\n The connection to server " + popServer + " is in state " +
                                        pop3ConnectionState);
            }
        }
        ...
        public void Connect()
        {
            ...
            else
            {
                ...
                {
                    throw new Pop3Exception("Connection to server " + popServer + ", port " + port +
                                            " failed.\nRuntime Error: " + ex);
                }
                ...
                if (useSSL)
                {
                    ...
                    {
                        throw new Pop3Exception("Server " + popServer +
                                                " found, but cannot get SSL data stream.\nRuntime Error: " + ex);
                    }
                    ...
                    {
                        throw new Pop3Exception("Server " + popServer +
                                                " found, but problem with SSL Authentication.\nRuntime Error: " + ex);
                    }
                }
                else
                {
                    ...
                    {
                        throw new Pop3Exception("Server " + popServer +
                                                " found, but cannot get data stream (without SSL).\nRuntime Error: " +
                                                ex);
                    }
                }
                ...
                catch (Exception ex)
                {
                    ...
                    {
                        throw new Pop3Exception("Server " + popServer +
                                                " found, but cannot read from SSL stream.\nRuntime Error: " + ex);
                    }
                    else
                    {
                        throw new Pop3Exception("Server " + popServer +
                                                " found, but cannot read from stream (without SSL).\nRuntime Error: " +
                                                ex);
                    }
                }
                ...
                {
                    throw new Pop3Exception("Server " + popServer + " doesn't accept password '" + password +
                                            "' for user '" + username + "'.\nMessage: " + response);
                }
        ...
        public int GetEmailSize(int msg_number)
        {
            EnsureState(Pop3ConnectionStateEnum.Connected);
            string response;
            executeCommand("LIST " + msg_number, out response);
            int EmailSize = 0;
        ...
        protected bool SendRetrCommand(int MessageNo)
        {
            ...
            string response;
            if (!executeCommand("RETR " + MessageNo, out response))
            {
        ...
        private bool executeCommand(string command, out string response)
        {
            ...
            if (response == null)
            {
                isSupressThrow = executeReconnect(new IOException("Timeout", new SocketException(10053)), command,
                                                  commandBytes);
                if (isSupressThrow)
                {
                    //wait for response one more time
