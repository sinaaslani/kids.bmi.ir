...
namespace BMISSOClientHelper
{
    public class SSOClientHelper
    {
        ...
        public delegate T FillUser<out T>(UserProfile User);
        public delegate string Srializer<in T>(T obj);
        public delegate T DeSrializer<out T>(string SerialStr);

        public static bool IsBMIUserLoggedIn<T>(out T User, FillUser<T> FillAction,
        ...
        public void LogOutCurrentUser(string ReturnUrl, bool GoToSSO = true, SessionLogoutAction Action = SessionLogoutAction.Abandon)
        {
            ClearLoginCoockie(HttpContext.Current.Request.Url.Host);
